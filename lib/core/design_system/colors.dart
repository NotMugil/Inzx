...
class MineColors {
  ...
  static Color contrastTextOn(Color background) {
    final luminance = background.computeLuminance();
    return luminance > 0.5 ? Colors.black : Colors.white;
  }

  /// Get adaptive text colors based on background luminance
  /// Returns (primary, secondary, tertiary) colors
  static ({Color primary, Color secondary, Color tertiary}) adaptiveTextColors(
    Color background,
  ) {
    final luminance = background.computeLuminance();
    // Threshold for switching to light text - if bg is darker than ~0.45 luminance
    if (luminance < 0.45) {
      return (
